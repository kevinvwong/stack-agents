...
| [agent-lifecycle.md](agent-lifecycle.md) | `agent-lifecycle` | Owns the `/agents:*` lifecycle commands (hire, fire, train, combine, review). Treats the roster as a workforce; mutates agent files as a normal operation. |
...
7. **Scaffold agents only:** If the agent writes or rewrites files (scaffold/bootstrap operations), add an `## Isolation` section documenting `isolation: worktree` usage — see `agents/project-setup.md` for the standard pattern.
