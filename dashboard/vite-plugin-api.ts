...
function getGitInfo(projectPath: string): GitInfo {
  ...
  const lastCommitDate =
    runGit(["log", "-1", "--format=%cI", "--no-merges"], projectPath) ||
    undefined;
...
function detectStack(projectPath: string): string[] {
  ...
  try {
    ...
    if (deps["@neondatabase/serverless"] || deps["@vercel/postgres"])
      stack.push("Neon");
    if (deps["@clerk/nextjs"] || deps["@clerk/clerk-sdk-node"])
      stack.push("Clerk");
    if (deps["@anthropic-ai/sdk"]) stack.push("Claude API");
    if (deps["elevenlabs"] || deps["@elevenlabs/elevenlabs-js"])
      stack.push("ElevenLabs");
  ...
  if (existsSync(pkgPath)) {
    try {
      ...
      if (portMatch) {
        ...
        const defaultPort = stack.includes("Next.js")
          ? 3000
