...
// #121: 'issues' was dead — no <option> rendered it and the sort fell through to 0.
// #101: 'commit' removed — vite-plugin-api.ts only exposes pre-formatted strings like
// "abc1234 commit message (2d ago)", so localeCompare sorted by SHA prefix, not date.
// Adding a real `committedAt: number` requires changes to the API plugin (out of scope
// for this PR); until then, removing the broken option is the honest fix.
type SortKey = "name";

const STACK_COLORS: Record<string, string> = {
  // #102: '#000' on the #1e293b card background was unreadable. Switched to a light
  // slate that has the same role as other text on this card (color: '#f1f5f9' in active
  // state still wins — this is the resting fg).
  "Next.js": "#e2e8f0",
  React: "#61dafb",
  "Drizzle ORM": "#c5f7ae",
  Neon: "#00e5b5",
...
function VercelBadge({ projectId }: { projectId: string }) {
  ...
  // #109: distinguish "fetch failed" from "not deployed yet". When the catch fires
  // we render a muted "▲ —" instead of returning null, so deploy issues are visible
  // rather than silently masquerading as "this project has no Vercel project".
...
function ProjectCard({
  ...
  // #100: the old catch did `setIssues([]); setExpanded(true)`, which made fetch
  // failures look identical to "no open issues". Track the error explicitly and
  // expose a retry path.
    ...
    >
      ...
      >
        ...
        <button
          ...
          title={
            issuesError
              ? "Failed to load issues — click to retry"
              : issues === null
...
export function ProjectDashboard() {
  ...
  // #121 bonus: reflect the active tab in document.title so browser tab labels
  // distinguish this view. Restore the prior title on unmount.
  useEffect(() => {
  ...
  // Sort — currently only 'name' (see SortKey comment above for #101 rationale).
    ...
    >
      ...
      >
        <div style={{ display: "flex", gap: 8 }}>
          ...
          />
          {/* Sort dropdown — 'commit' removed (#101), 'issues' never existed (#121) */}
          <select
            value={sortKey}
      ...
      {/* Card grid area — loading state lives here, NOT in the toolbar (#111) */}
      <div
        style={{
          flex: 1,
          overflow: "auto",
