    ...
    if (chain) {
      const chainIdOf = (stem: string) => {
        const map: Record<string, string> = {
          'web-data': 'data', 'web-security': 'security', 'web-ai-llm': 'ai-llm',
          'web-application': 'application', 'web-infrastructure': 'infrastructure',
          'web-observability': 'observability', 'web-presentation': 'presentation',
          'game-narrative': 'narrative', 'game-level-design': 'level-design', 'game-production': 'production',
        }
...
function AgentNode({ data, selected }: { data: { agent: AgentMeta }; selected: boolean }) {
  const { agent } = data
  ...
  return (
    ...
    >
      ...
      <div style={{ fontWeight: 700, fontSize: 11, color: selected ? '#fff' : color, marginBottom: 2 }}>
        {agent.family}
      </div>
...
export function AgentGraph({ agents, edges, selectedAgent, onSelectAgent }: Props) {
  ...
  const initialEdges: FlowEdge[] = useMemo(
    () =>
      edges.map((e, i) => ({
        ...
        markerEnd: { type: MarkerType.ArrowClosed, color: e.type === 'chain' ? '#94a3b8' : '#f59e0b' },
  ...
  return (
    <div style={{ width: '100%', height: '100%', background: '#0f172a' }}>
      ...
      >
        ...
        <Controls />
        <MiniMap nodeColor={(n) => FAMILY_COLORS[(n.data as { agent: AgentMeta }).agent.family] ?? '#666'} maskColor="#0f172a99" />
      </ReactFlow>
    </div>
  )
}
