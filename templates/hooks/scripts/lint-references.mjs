...
//
// Usage:
//   node lint-references.mjs                      # lint cwd (or script dir if invoked there)
//   node lint-references.mjs --root <path>        # lint a specific repo root
//   node lint-references.mjs --json               # emit JSON for CI consumption
//   node lint-references.mjs --quiet              # print only errors
//
// When invoked outside a stack-agents-style repo (no agents/ and no commands/
// in --root), the script exits 0 silently — safe to wire into a hook that
// runs in any project.
//
...
//   1. --root <path> explicit
//   2. cwd if it looks like a stack-agents repo (has agents/ AND commands/)
//   3. script's parent dir (the original behavior — works when invoked from
//      inside the stack-agents repo itself)
function resolveRoot() {
  const explicit = getFlag("--root");
  if (explicit) return resolve(explicit);
  const cwd = process.cwd();
  const looksRight = (d) => {
    try {
      return statSync(join(d, "agents")).isDirectory() && statSync(join(d, "commands")).isDirectory();
    } catch { return false; }
  };
  if (looksRight(cwd)) return cwd;
  const scriptParent = resolve(new URL("..", import.meta.url).pathname);
  if (looksRight(scriptParent)) return scriptParent;
  return null;  // no valid target — caller will no-op
}

const ROOT = resolveRoot();
if (!ROOT) {
  if (!QUIET && !JSON_MODE) console.log("lint-references: no agents/ + commands/ found in cwd or script dir — skipping.");
  if (JSON_MODE) console.log(JSON.stringify({ skipped: true, reason: "no lint targets" }));
  process.exit(0);
}
...
function stripCodeBlocks(body) {
  // Remove fenced code blocks first, then inline code spans, so references
  // shown as illustrative examples don't trigger false positives.
  return body
    .replace(/```[\s\S]*?```/g, "")
    .replace(/`[^`\n]*`/g, "");
}
